...
class HotelListView(generics.ListAPIView):
    ...
    filter_fields = ['city', 'pin', 'star_category', 'amenities', 'tax_type', 'tax_percent_min', 'tax_percent_max', 'total_rooms_min', 'total_rooms_max',
                     'available_rooms_min', 'available_rooms_max', 'price_min', 'price_max', 'available_from_after', 'available_from_before', 'available_to_after', 'available_to_before']
...
class CarListView(generics.ListAPIView):
    ...
    filter_fields = ['car_type', 'fuel_type', 'seats_min', 'seats_max', 'transmission', 'ac', 'bags', 'price_min',
                     'price_max', 'origin_city', 'destination_city', 'available_from_after', 'available_from_before']
...
class FlightListView(generics.ListAPIView):
    ...
    filter_fields = ['departure_airport_city', 'arrival_airport_city', 'departure_time_after', 'departure_time_before',
                     'arrival_time_after', 'arrival_time_before', 'available_seats_min', 'available_seats_max', 'price_min', 'price_max']
...
class BusListView(generics.ListAPIView):
    ...
    filter_fields = ['departure_station', 'arrival_station', 'departure_date_after', 'departure_date_before', 'departure_time_after',
                     'departure_time_before', 'arrival_date_after', 'arrival_date_before', 'arrival_time_after', 'arrival_time_before']
...
class PackageListView(generics.ListAPIView):
    ...
    filter_fields = ['origin_city', 'destination_city', 'activities', 'departure_after',
                     'departure_before', 'star_category', 'price_min', 'price_max', 'with_flights', 'total_rooms']
...
class PaymentView(APIView):
    ...
    def post(self, request):
        ...
        if package_type == 'hotel':
            ...
            reservation = HotelReservation.objects.create(user=user, hotel=room.hotel, room=room, total_guests=people,
                                                          check_in_date=startDate, check_out_date=endDate, contact_email=contact_email, contact_phone=contact_phone)
            ...
            reservation = CarReservation.objects.create(user=user, car=car, passenger=people, contact_email=contact_email,
                                                        contact_phone=contact_phone, rental_start_date=startDate, rental_end_date=endDate)
            ...
            reservation = FlightReservation.objects.create(
                user=user, flight=flight, passenger=people, contact_email=contact_email, contact_phone=contact_phone, departure_on=startDate)
            ...
            reservation = BusReservation.objects.create(
                user=user, bus=bus, passenger=people, contact_email=contact_email, contact_phone=contact_phone, departure_on=startDate)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = PackageReservation.objects.create(
                user=user, package=package, passenger=people, contact_email=contact_email, contact_phone=contact_phone)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = YachtReservation.objects.create(
                user=user, yacht=package, passenger=people, contact_email=contact_email, contact_phone=contact_phone)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = ThemeParkReservation.objects.create(
                user=user, theme_park=package, admission_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = TopAttractionReservation.objects.create(
                user=user, top_attraction=package, ticket_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = DesertSafariReservation.objects.create(
                user=user, desert_safari=package, participant_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = WaterParkReservation.objects.create(
                user=user, water_park=package, admission_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = WaterActivityReservation.objects.create(
                user=user, water_activity=package, participant_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = AdventureTourReservation.objects.create(
                user=user, adventure_tour=package, participant_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = ComboTourReservation.objects.create(
                user=user, combo_tour=package, participant_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
            ...
            try:
                ...
                return Response({'error': 'Invalid package id'}, status=status.HTTP_404_NOT_FOUND)
            reservation = DubaiActivityReservation.objects.create(
                user=user, dubai_activity=package, participant_count=people, contact_email=contact_email, contact_phone=contact_phone)
            amount = package.price * int(people)
        ...
        try:
            ...
            p_billing_address = request.data.get(
                'billing_address', 'mali mohalla')
            p_billing_city = request.data.get('billing_city', 'Maheshwar')
            p_billing_state = request.data.get(
            ...
            p_billing_email = request.data.get(
                'billing_email', 'rohanphulkar936@gmail.com')
        ...
        '''
            ...
            fin = Template(html).safe_substitute(
                mid=p_merchant_id, encReq=encryption, xscode=accessCode)
    ...
    if request.method == 'POST':
        try:
            ...
            if order_status_element and order_id_element:
                order_status = order_status_element.find_next(
                    'td').get_text().strip()
            ...
            if order_status == 'Success':
                ...
                if booking.hotel:
                    ...
                    flight = FlightReservation.objects.get(
                    ...
                    package = PackageReservation.objects.get(
                    ...
                    package = YachtReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = ThemeParkReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = TopAttractionReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = DesertSafariReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = WaterParkReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = WaterActivityReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = AdventureTourReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = ComboTourReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                    ...
                    package = DubaiActivityReservation.objects.get(
                        id=booking.package.id)
                    package.status = 'confirmed'
                    email = package.contact_email
                    phone = package.contact_phone
                ...
                booking.pdf.save(f'{uuid.uuid4()}.pdf',
...
def get_model_instances(request):
    ...
    instance_data = [{'pk': instance.pk, 'str': str(
...
def duplicate_instance(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(int(count)):
            ...
            new_instance.save()
        messages.success(
            request, f"{count} entries of {instance} has been created.")
...
class SightseeingList(generics.ListAPIView):
    queryset = Sightseeing.objects.all()
    serializer_class = SightSeeingSerializer


class SightseeingDetail(generics.RetrieveAPIView):
    queryset = Sightseeing.objects.all()
    serializer_class = SightSeeingSerializer


class DhowCruiseDubaiList(generics.ListAPIView):
    queryset = DhowCruiseDubai.objects.all()
    serializer_class = DhowCruiseDubaiSerializer


class DhowCruiseDubaiDetail(generics.RetrieveAPIView):
    queryset = DhowCruiseDubai.objects.all()
    serializer_class = DhowCruiseDubaiSerializer


class GlampingToursList(generics.ListAPIView):
    queryset = GlampingTours.objects.all()
    serializer_class = GlampingTourSerializer


class GlampingToursDetail(generics.RetrieveAPIView):
    queryset = GlampingTours.objects.all()
    serializer_class = GlampingTourSerializer


class BurjKhalifaToursList(generics.ListAPIView):
    queryset = BurjKhalifaTours.objects.all()
    serializer_class = BurjKhalifaToursSerializer


class BurjKhalifaToursDetail(generics.RetrieveAPIView):
    queryset = BurjKhalifaTours.objects.all()
    serializer_class = BurjKhalifaToursSerializer


class SkyAdventuresList(generics.ListAPIView):
